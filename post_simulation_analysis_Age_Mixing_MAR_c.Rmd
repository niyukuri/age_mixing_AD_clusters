...
Inferring age mixing from transmission clusters in a scenario of data missingess, where an individual's missingness depends on his/her gender. In the following scenario, we assume that women represent at most 50% of selected individuals in the sample.
...
The error for WSD.male and SDAD.male parameters varies between 0 and 0.1 (WSD.male being on top), AAD.male varies between 0.125 and 0.3 with a decrease when the squence coverage increase, BSD.male and slope.male varies between 0.3 and 0.5 (slope being on top) with the slope being on top and both showing an sharp decrease with an increase of sequence coverage, and the intercept.male varies between 0.615 and 1.15 with fast/sharp decrease as the sequence coverage increase compared to other measurements.
...
As the sequence coverage increase the number of pairings becomes close to the true values at 100% coverage. But also, there are some differences compared to MCAR scenarios, for example the number of pairings in M.25.40.F.15.25 (7) decrease at 95% coverage compared to MCAR scenario (10), the main explanation may be squewness due to gender in sampling strategy. Same effect will be observed for proportions of pairings.
