...
Inferring age mixing from transmission clusters in a scenario of data missingess, where an individual's missingness depends on his/her gender. In the following scenario, we assume that women represent at most 30% of selected individuals in the sample.
...
We can see that the average age difference from infected individuals in their partnership is far (around 8 years) from the mean of preferred age differences distribution for men/women which was 10 from 35% to 45% of sequence coverage, the difference between the two is less than 1. But from 50% of sequence coverage can see that the average age difference from infected individuals in their partnership is not far  (greater than 9 years) from the mean of preferred age differences distribution for men/women which was 10, the difference between the two is less than 1. But the standard deviation of age difference in selected infected individuals did not chnage much (around 6 years) compared to standard deviation of preferred age differences distribution for men, which is years; the difference between the two is slightly greater than 1.
...
The error for WSD.male and SDAD.male parameters varies between 0 and 0.12 (WSD.male being on top), AAD.male varies between 0.125 and 0.25 with a decrease when the squence coverage increase, BSD.male and slope.male varies between 0.25 and 0.50 (slope being on top) with the slope being on top and both showing an sharp decrease with an increase of sequence coverage, and the intercept.male varies between 0.5 and 1.0 with fast/sharp decrease as the sequence coverage increase compared to other measurements.
...
As the sequence coverage increase the number of pairings becomes close to the true values at 100% coverage. But also, there are some differences compared to MCAR scenarios, for example the number of pairings in M.25.40.F.15.25 (5) decrease at 95% coverage compared to MCAR scenario (10), the main explanation may be squewness due to gender in sampling strategy. Same effect will be observed for proportions of pairings.
